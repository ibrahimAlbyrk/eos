...
// is downgraded to a plain-cwd spawn in the source checkout. A worker type may
...
// worktrees-disabled setting always wins over a type's "worktree" preference.
...
export function resolveSpawnIsolation(
  ...
  opts: { worktreesDisabled: boolean; typeIsolation?: "worktree" | "cwd" },
): SpawnIsolation {
  ...
  // or the type asks for cwd isolation.
  if ((opts.worktreesDisabled || opts.typeIsolation === "cwd") && spec.worktreeFrom) {
    return { cwd: spec.worktreeFrom, worktreeFrom: undefined };
  }
  // A type that wants a worktree promotes a plain-cwd spawn into an isolated
  // tree — but never when the user disabled worktrees globally (that wins).
  if (!opts.worktreesDisabled && opts.typeIsolation === "worktree" && spec.cwd && !spec.worktreeFrom) {
