...
// claude / codex follow. This replaces the ProcessSupervisor + PortAllocator
...
export interface AgentCapabilities {
   ...
   *  claude's query() exposes no fork/rewind primitive → false. The rewind
   ...
   *  (claude, in-process). The UI gates its live thinking renderer on this
   ...
   *  (claude: options.resume). Drives boot reconciliation resume-vs-suspend. */
  readonly resumable?: boolean;
  /** True when the backend can reset its live conversation/context in place (the
   *  `/clear` slash command). claude-cli: native /clear over the PTY; claude:
...
// What the UI model picker shows for a provider. claude-cli + claude share
...
export interface BackendDescriptor {
  ...
  readonly label: string;                                   // UI: "Claude"
  ...
  // can hand its conversation off between them on resume — claude-cli ↔ claude
...
// directly; the structured lanes (claude, in-process) go through here.
...
export interface AgentSession {
  ...
  // claude: query() has no fork primitive, so the session omits them.
  ...
  // leaks into neither the next turn nor a resume. claude: relaunch the
  ...
  // effort switching has no lever (claude: start()/resume apply effort via
