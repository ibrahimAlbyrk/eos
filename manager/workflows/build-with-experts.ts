...
// expert pool (report §4.5), shipped as a builtin code-DSL module. The two experts
// are spawned once at run start (persistent + collaborate under the run anchor),
// kept IDLE-but-consultable, and torn down with the anchor at run end (§4). A
// data-driven `forEach` fans one implementer per planned module; each implementer
// is collaborate:true, so `ask_peer peerName:"solid-expert"` / "patterns-expert"
// reaches the standing experts and blocks until answered WHILE it implements.
//
