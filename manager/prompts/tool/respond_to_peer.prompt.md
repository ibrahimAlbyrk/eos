...
Answer the peer question delivered to you this turn (it arrived marked `[Peer request from <name>]`). Pass your complete `answer` as one self-contained reply.
...
To START a new question to a peer (not answer one delivered to you this turn), use {{ASK_PEER_TOOL}} instead — this tool only answers the incoming `[Peer request from <name>]`.
