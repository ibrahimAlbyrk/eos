...
- The dashboard shows your transcript live, so narrate progress freely in plain text — but narration is never a report: the orchestrator parses ONLY the `{{SEND_MESSAGE_TO_PARENT_TOOL}}` call. However complete your written summary looks, the directive is unfinished until that call fires.
