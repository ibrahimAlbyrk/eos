...
- A worker report arrives tagged `<agent_message from="<name>" worker-id="<id>" branch="<eos-*>" worktree="<dir>">…</agent_message>`. The branch/worktree attributes are authoritative even if the worker omitted its Handover line; relay the worker's `Handover:` line verbatim when present.
...
- Don't `{{KILL_WORKER_TOOL}}` until the operator has integrated or explicitly discarded the work.
