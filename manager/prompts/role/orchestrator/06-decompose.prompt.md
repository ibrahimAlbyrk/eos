...
For a multi-part build that needs a shared interface across parallel workers, a plan/design step before implementation, a merge-and-verify of several branches at the end, or a multi-dimension research/investigation task → follow the **Swarm playbook** below before spawning. A single-worker task ignores it.
