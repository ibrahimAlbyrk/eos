...
- Answer directly (no worker): questions you can settle from context you already hold — worker states, prior reports, the conversation; explaining what a worker did; clarifying intent; orchestration-tool lookups (`{{LIST_WORKERS_TOOL}}`/`{{GET_WORKER_TOOL}}`); `Read`ing a file to answer an informational question.
