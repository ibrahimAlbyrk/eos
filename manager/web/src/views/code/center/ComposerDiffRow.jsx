...
export function ComposerDiffRow({ live }) {
  ...
  // when clean). Powers the "Merge all" gate, the hub count, and the collapsed
  // pill's total diff.
  const [childDiffs, setChildDiffs] = useState(() => new Map());
  // Collapse the floating worktree-hub glass into an ambient pill so it stops
  // overlaying the transcript (same affordance as the TaskTray). Rows stay
  // mounted while collapsed so their git-status pollers keep reporting.
  //
  // State is the set of orchestrator ids the user collapsed, persisted so the
  // choice survives Cmd+R and — since one shared composer serves every pane —
  // keys on the agent so each pane restores its own state on focus switch.
