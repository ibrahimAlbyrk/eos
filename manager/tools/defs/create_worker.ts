...
export const createWorkerDef: ToolDefinition = {
  ...
  inputSchema: {
    name: z.string().describe("Identity, kebab-case (e.g. 'perf-profiler'). Spawn it with spawn_worker({ from: <name> })."),
    ...
    body: z.string().describe("This worker's role instructions, composed ALONGSIDE the standard worker contract — do NOT restate the result:/needs input:/failed: signal protocol, report structure, or Handover. Task-specific only: environment map, output contract, if-then failure rules. (See create_worker's full guidance.)"),
  },
