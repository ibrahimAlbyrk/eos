...
export class SqliteWorkerRepo implements WorkerRepo {
  ...
  constructor(db: DatabaseSync) {
    ...
    this.stmtInsert = db.prepare(`
      INSERT INTO workers (id, state, cwd, worktree_from, branch, prompt, name, pid, port, started_at, parent_id, model, effort, is_orchestrator, backend_kind, backend_profile, agent_role, worker_type, with_gateway, collaborate, turn_started_at, worktree_dir, workspace_owner_id, workspace_ready)
      VALUES (?, 'SPAWNING', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
