...
import { getSnapshot, subscribe, revalidate, loadPatch, notifyActivity, pruneExcept } from "./diffStore.js";
...
describe("diffStore", () => {
  ...
  it("pruneExcept drops entries for workers absent from the live set", async () => {
    const keep = freshId();
    const gone = freshId();
    await revalidate(keep);
    await revalidate(gone);
    expect(getSnapshot(gone).changes).toEqual(CHANGES);
    pruneExcept(new Set([keep]));
    expect(getSnapshot(gone).changes).toBeNull();
    expect(getSnapshot(keep).changes).toEqual(CHANGES);
  });
